...
// “E2EEncloud” || “Padding”
const saltString =
  defaultString + 'P'.repeat(saltStringMaxLength - defaultString.length);
...
export const createSalt = (ClientRandomValue: Uint8Array) => {
  ...
  const concatenatedSaltArray = new Uint8Array(
    saltStringMaxLength + ClientRandomValue.length,
  );
...
export const generateRSAKey = async (masterkey: CryptoKey) => {
  ...
  const [privateKeyArrayBuffer, publicKeyArrayBuffer] = await Promise.all([
...
export const importRSAKey = async (params: {
  masterkey: CryptoKey;
