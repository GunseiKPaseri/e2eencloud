...
export const decryptByRSA = (data: ArrayBuffer): Promise<ArrayBuffer> => {
  if (!rsaPrivateKey)
    return Promise.reject(new Error('there is no rsa privatekey'));
...
export const encryptByRSA = (data: ArrayBuffer): Promise<ArrayBuffer> => {
  if (!rsaPublicKey)
    return Promise.reject(new Error('there is no rsa publickey'));
...
export const decryptoByDerivedEncryptionKey = (params: {
  data: ArrayBuffer;
  iv: Uint8Array;
}) => {
  if (!DerivedEncryptionKey)
    return Promise.reject(new Error('there is no DerivedEncryptionKey'));
  return decryptAESCTR(params.data, DerivedEncryptionKey, params.iv);
};
