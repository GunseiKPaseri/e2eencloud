...
type EmailConfirmReturned = (
  | { success: false }
  | { success: true; user: UserState }
) & {
  token: string;
};
...
>('auth/confirm_email', async (usertoken, { dispatch }) => {
  try {
    // generate user

    // 128 bit MasterKey
    const MasterKey = window.crypto.getRandomValues(
      new Uint8Array(AES_AUTH_KEY_LENGTH),
    );
    // 128 bit Client Random Value
    const ClientRandomValue = window.crypto.getRandomValues(
      new Uint8Array(AES_AUTH_KEY_LENGTH),
    );
    // 256 bit Salt
    ...
    const DerivedEncryptionKey = await getAESCTRKey(
      DerivedKey.slice(0, AES_AUTH_KEY_LENGTH),
    );
...
export const afterConfirmEmailAsyncFullfilled: CaseReducer<
  AuthState,
  PayloadAction<EmailConfirmReturned>
> = (state, action) => {
  if (action.payload.success) {
    state.user = action.payload.user;
  }
  state.confirmstate[action.payload.token] = action.payload.success
    ? 'SUCCESS'
    : 'ERROR';
};
