...
import {
  serializeTags,
  deserializeTags,
} from '~/features/file/util/serializeTags';
...
function FileGrid({
  ...
  sx?: SxProps<Theme>;
  nodeRef?: React.DetailedHTMLProps<
  ...
  const selectionModel: GridInputRowSelectionModel =
    activeFileGroup?.selecting ?? [];
      ...
      <div style={{ height: 300, width: '100%' }} ref={nodeRef}>
        <DataGrid
          ...
          onRowSelectionModelChange={(newSelectionModel) => {
            if (!$array($string)(newSelectionModel))
              throw new Error('Unexpected Error!');
            dispatch(changeSelection({ selection: newSelectionModel }));
          }}
          rows={activeFileGroup.files.map<GridRowsProp[number]>((x) => {
