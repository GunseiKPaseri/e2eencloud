...
import { type SearchQueryForRedux, type SearchQuerySetForRedux } from '../../../util/search.type';
import KeyWordChip, { type QueryKeyWordChipProps } from '../../atom/search/QueryKeyWordChip';
...
import { hasById, indexOfById, searchTerm, searchTermById } from '~/features/file/util/search';

type KeyWordChipSortableProps = Omit<QueryKeyWordChipProps, 'isDragging'> & {id: string}
  ...
  return (
    <div
      ...
      style={{
        ...
        zIndex: isDragging ? 1 : 0, position: 'relative',
...
const QuerySetBuilder = ({querySet, onDelete, parentIgnore, ...props}: QuerySetBuilderProps) => {
...
const AddButton = (props: {onClick: MouseEventHandler<HTMLButtonElement>, disabled?: boolean}) => {
  return (
    <Button
...
const Placeholder = (props: {children: ReactElement}) =>{
  const {
    isOver,
    setNodeRef,
  } = useDroppable({id: PLACEHOLDER_ID, })
...
function LichSearchInput(props: {onOpenAddQueryDialog: (key: string) => void}) {
  ...
  const searchQueryItemOrigin = activeFileGroup?.type === 'search' ? activeFileGroup.query : null
  ...
  const [dndActiveItem, setDndActiveItem] = useState<UniqueIdentifier | null>(null)
  ...
  const [clonedItems, setClonedItems] = useState<SearchQueryForRedux | null>(null);

  const changeQuery = (afterQuery: SearchQueryForRedux) => {
    dispatch(
      changeActiveFileGroupSearch({query: afterQuery})
    );
  }
  ...
  const AddQueryButton = (props: {targetQuery: string, disabled?: boolean}) => (
    <AddButton disabled={props.disabled} onClick={() => onOpenAddQueryDialog(props.targetQuery)} />
  )
  const OrSectionBuilder = (
    {andSection, onRefresh, parentIgnore}: {
      ...
      const onQuerySetDelete = (deletedQuerySet: SearchQuerySetForRedux) => {
        onRefresh({
          ...andSection,
          term: andSection.term.filter((querySet) => querySet.id !== deletedQuerySet.id)
        })
      }
      const ignore = parentIgnore || (andSection.ignore ?? false);
      const dndActiveTerm = typeof dndActiveItem === 'string' ? searchTermById(andSection, dndActiveItem) : undefined
      return (
        <TermGroupBox
          disabled={ignore}
          header="以下の全てを満たす"
        ...
        >
          ...
          >
            ...
            {
              andSection.term.length === 0 ? <div style={{width: 10, height: 50}}></div> : <></>
            }
          </SortableContext>
        </TermGroupBox>
      )
        ...
        searchQueryItem === null || searchQueryItem.term.length === 0 ? (
          ...
          <DndContext
            ...
            onDragOver={({active, over}) => {
              ...
              if (!overId  || overId === TRASH_ID || searchTermById(searchQueryItem, active.id)) {
                return
              }
              ...
              if(overOrTerm !== activeOrTerm) {
                setSearchQueryItem((beforeSearchQuery) => {
                  const activeTerm = beforeSearchQuery && searchTermById(beforeSearchQuery, activeOrTerm.id)
                  const overTerm = beforeSearchQuery && searchTermById(beforeSearchQuery, overOrTerm.id) 
                  if(!overTerm || !activeTerm) return null
                  const activeIndex = activeTerm ? indexOfById(activeTerm, active.id) : -1
                  ...
                  if (hasById(overTerm, overId)) {
                    ...
                    newIndex =
                      overIndex >= 0 ? overIndex + modifier : overTerm.term.length + 1;
                  }
                  ...
                  return {
                    ...
                    term: beforeSearchQuery.term.map(orTerm => {
                      if(orTerm.id === activeTerm.id){
                        return {
                          ...orTerm,
                          term: orTerm.term.filter(andTerm => andTerm.id !== active.id)
                        }
            ...
            onDragEnd={({active, over}) => {
              ...
              if (overContainer) {
                ...
                const afterSearchQueryItem = (activeIndex !== overIndex ? (searchQueryItem ? {
                  ...searchQueryItem,
                  term: searchQueryItem.term.map(x => (x.id === overContainer.id ? {
                    ...x,
                    term: arrayMove(x.term, activeIndex, overIndex)
                  } : x))
                } : null) : searchQueryItem)
          ...
          >
            <TermGroupBox
              disabled={ignore}
              header="以下のいずれかを満たす"
              addButton={<Placeholder><AddQueryButton targetQuery='' /></Placeholder>}
            >
              {
...
function SearchInput(props: {sx?: SxProps<Theme>}) {
  ...
  return (
    <>
      ...
      <Dialog onClose={() => setOpenSearchDialog(false)} open={openSearchDialog}>
        <LichSearchInput
        ...
        <TextField
          ...
          value={activeFileGroup?.type === 'search' ? activeFileGroup.queryString : ''}
          variant="standard"
          onChange={(e) => {
      ...
      <AddQueryDialog onAddQuery={(query) => {
          const newQuery: SearchQueryForRedux['term'] = activeFileGroup?.type == 'search'
          ? activeFileGroup.query.term.map(x => ({
              ...x,
              term: (
                x.id === addQueryDialogTarget
                  ? [...x.term, query]
                  : x.term
              )
            }))
          : [{term: [], id: 'added'}]
          dispatch(
            changeActiveFileGroupSearch({query: {
                ...query,
                term: addQueryDialogTarget === ''
