...
const getDir = (
  ...
  root.files.map((id): Promise<Item> | Promise<Item>[] => {
...
const genZipFile = async (root: FileNode<FileInfoFolder>, fileTable: FileTable) => {
  // get all file
  const children = getFileChildren(root, fileTable);
  await Promise.all(children.map(
    (id) => store.dispatch(filedownloadAsync({ fileId: id, active: false })),
  ));
  ...
  await writer.add(`${newRoot.name}.meta.json`, new zip.TextReader(JSON.stringify(metadata)));
