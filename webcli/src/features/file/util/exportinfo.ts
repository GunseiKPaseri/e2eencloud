 ...
 * @returns Export File
 */
export const exportFileInfo = (target: FileNode<FileInfoFile>): ExportFileType => (
  ({
    id, name, createdAt, version, sha256, mime, size, tag, expansion,
  }) => (
    {
      id, name, createdAt, version, sha256, mime, size, tag, expansion: trimExpansion(expansion),
    }
  ))(target);
