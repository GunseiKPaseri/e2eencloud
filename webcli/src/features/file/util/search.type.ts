...
export const strSearchTypeValidator = $union([
  $const('eq'),
  $const('in'),
  $const('inlike'),
]);
...
export const numberSearchTypeValidator = $union([
  $const('>'),
  $const('<'),
  $const('>='),
  $const('<='),
...
export const searchQuerySetTypeValidator = $union([
  $const('name'),
  $const('mime'),
  $const('tag'),
  $const('size'),
...
export type SearchQuerySetForRedux =
  | SearchQuerySetPrimitiveOnly
...
export type SearchQueryTermForReduxOmitId = DistributiveOmit<
  SearchQuerySetForRedux,
  'id'
>;
export type SearchQueryAndTermForReduxOmitId = DistributiveOmit<
  SearchQueryForRedux['term'][number],
  'id' | 'term'
> & { term: SearchQueryTermForReduxOmitId[] };
export type SearchQueryForReduxOmitId = DistributiveOmit<
  SearchQueryForRedux,
  'id' | 'term'
> & {
  term: SearchQueryAndTermForReduxOmitId[];
};
