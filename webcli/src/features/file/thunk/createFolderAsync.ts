...
{ state: RootState }>(
  ...
  async ({ name }, { getState, dispatch }) => {
    ...
    const parent = activeFileGroup.parents.length === 0
      ? null
...
CaseReducer<FileState, PayloadAction<CreateFolderAsyncResult>> = (state, action) => {
  ...
  // 作成したフォルダをstateに反映
  const parent:string = parents.length === 0
    ? 'root'
  ...
  const newFileTable:FileTable = {
    [fileInfo.id]: {
      ...fileInfo, files: [], history: [fileInfo.id], origin: uploaded,
    },
    ...state.fileTable,
  };
    // add parent
  ...
  state.activeFileGroup = {
    type: 'dir', folderId: parent, files: sortedNewFiles, selecting: [], parents,
  };
};
